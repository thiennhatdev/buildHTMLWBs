...
(() => {
  ...
  requireTask(`${cfg.task.buildSass}`, `./${cfg.folder.tasks}/`, {
    src: cfg.folder.src,
    dest: cfg.folder.build,
    mainSass: cfg.file.mainSass,
    mainSassMin: cfg.file.mainSassMin,
  ...
  /**
   * Compile sass files listed in the config
   */
  ...
  requireTask(`${cfg.task.buildStylesVendors}`, `./${cfg.folder.tasks}/`, {
    src: cfg.folder.src,
    dest: cfg.folder.build,
    vendorSass: cfg.file.vendorSass,
    vendorSassMin: cfg.file.vendorSassMin,
    showError: showError
  });
   ...
   * Fonts
   */
  requireTask(`${cfg.task.fonts}`, `./${cfg.folder.tasks}/`, {
    src: cfg.folder.src,
    dest: cfg.folder.build
  });
  ...
  requireTask(`${cfg.task.browserSync}`, `./${cfg.folder.tasks}/`, {
    mainHtml: 'assets/'+cfg.file.mainHtml,
